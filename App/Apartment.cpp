...
Apartment::Apartment(int id, std::string city, int capacity, std::vector<std::string> livingConditions, std::vector<std::string> bookingConditions, std::vector<std::string> amenities, std::vector<float> seasonalPricingPerWeek, int sellerId)
...
void Apartment::Deserialize(std::vector<std::string> params) {
    ...
    seasonalPricingPerWeek = HelperFuncs::separateLine<float>(params[6], '|', [](const std::string& i) { return std::stof(i); });
    sellerId = std::stoi(params[7]);
}

void Apartment::editContainer(std::vector<std::string>* container, const std::string& value, int index)
{
