...
template <ModelT T> struct TypeInfo
{
    ...
    TypeInfo(Container<T>& c, const std::string& f, int& id)
        : container(c), filename(f), lastId(id)
    {
    }
};

template <ModelT T> TypeInfo<T>& getTypeInfo()
{
    static_assert(!std::is_same_v<T, T>, "TypeInfo specialization missing for "
                                         "this type.");
}
