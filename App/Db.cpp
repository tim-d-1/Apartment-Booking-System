...
void Db::LoadContainer() {
    ...
    if (!storage->Exists(typeInfo.TypeFilename)) return /* fail */;
...
template<ModelT T>
void Db::Add(std::shared_ptr<T> obj) {
    if (obj->GetId() <= 0) {
...
template<ModelT T>
std::shared_ptr<T> Db::Search(Predicate<T> pred) {
...
template<ModelT T>
Container<T> Db::SearchAll(Predicate<T> pred) {
    Container<T> res;
    auto& cont = getTypeInfo<T>().TypeContainer;
    for (const auto& ptr : cont) {
...
template<ModelT T>
bool Db::Remove(Predicate<T> pred) {
    auto& cont = getTypeInfo<T>().TypeContainer;
...
template<ModelT T>
void Db::RemoveAll(Predicate<T> pred) {
    auto& cont = getTypeInfo<T>().TypeContainer;
